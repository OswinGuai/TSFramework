...
class Encoder(nn.Module):
    def __init__(self, layer_num, hidden_dim, r, r_hat, L, H, featureProjectionHidden):
        ...
        #self.first_encoder_layer = ResidualBlock(L + 0 + (L + H) * r_hat, hidden_dim, hidden_dim)
        self.first_encoder_layer = ResidualBlock(L + 0 + (L) * r_hat, hidden_dim, hidden_dim)
        self.other_encoder_layers = nn.ModuleList([
            ResidualBlock(hidden_dim, hidden_dim, hidden_dim) for _ in range(layer_num-1)
            ])

    def forward(self, x, covariates):       # , covariates, attributes
        ...
        # Concat
        e = torch.cat([x, covariates_flat], dim=1)  # [B*N,L+1+(L+H)*r_hat]
...
class Decoder(nn.Module):
    def __init__(self, layer_num, hidden_dim, r_hat, H, p, temporalDecoderHidden):
        ...
        self.temporaldecoder = ResidualBlock(p, temporalDecoderHidden, 1)
...
class TiDE(nn.Module):
    ...
    def forward(self, x):   # , covariates, attributes
        ...
        # Encoder
        e = self.encoder(x)  # , covariates, attributes

        # Decoder
        out = self.decoder(e)  # out: [B*N,H,1]     #  covariates_future
    ...
    def forward(self, x):  # , covariates, attributes
        # x: [B,L,N], covariates: [B,L+H,N,r], attributes: [B,N,1]
        batch_size = x.size(0)
        covariates = x[:,:,:-1].unsqueeze(-2) #[B, L, 1, C]
        x = x[:,:,-1].unsqueeze(-1)
        ...
        # Encoder
        e = self.encoder(x, covariates)  # , covariates, attributes
