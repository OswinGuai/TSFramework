...
class AluminaTransformerForecasting(GeneralForecasting):
    ...
    def eval_metric(self, eval_outputs, truth_values, epoch):
        ...
        mask = torch.where(truth_values.reshape([truth_values.shape[0]]) == -999, torch.zeros(truth_values.shape[0]), torch.ones(truth_values.shape[0]))
        if torch.sum(mask) == 0:
    ...
    def batch_loss(self, samples_batch, targets_batch, datetimes_batch, curr_iter):
        ...
        mask = torch.where(label_targets.reshape([label_targets.shape[0]]) == -999, torch.zeros(label_targets.shape[0]).to(self.device), torch.ones(label_targets.shape[0]).to(self.device))
        if torch.sum(mask) == 0:
            train_loss = reg_loss
